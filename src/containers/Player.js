...
import { controlGameIfFirst } from "../actions/host-action";
...
class Player extends Component {
  ...
  render() {
    const { chips, table, player } = this.props;
    ...
    return (
      <div className="player">
        {/* Details */}
        <h2>
          {player.name || player.id}({player.state || "idle"})
          <PlayerToken />
        </h2>
        ...
        {canBet && (
          <Fragment>
            ...
            {this.state.betMenuVisible ? (
              <BetMenu />
            ) : (
              <BetButton onClick={showBetMenu} />
            )}
          </Fragment>
        )}
